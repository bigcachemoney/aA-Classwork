...
def high_population_gdps
  ...
  FROM
    countries
  WHERE
    name IN ('China', 'India', 'Indonesia', 'United States of America')
  SQL
end
...
def population_in_millions
  ...
  FROM
    countries
  WHERE
    name IN ('Argentina', 'Bolivia', 'Brazil', 'Chile', 'Colombia', 'Ecuador', 'Guyana', 'Paraguay', 'Peru', 'Surinam', 'Uruguay', 'Venezuela')
  SQL
end
