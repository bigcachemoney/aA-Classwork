...
def sentence_swap(sentence, hash)
    ...
    #words.map {|word| hash.has_key?(word) ? hash[word] : word}.join(" ")
end
...
def triplet_true(str)
    ...
    while i < str.length - 2
        return true if str[i] == str[i+1] and str[i+1] == str[i+2] 
        i += 1
    end 
    false
end
...
# # every letter is repeated multiple times given based on the number that appears directly after the letter.
...
def uncompress(str)
    ...
    while i < chars.length
        new_str += chars[i] * chars[i+1].to_i
        i+=2
    end  
   new_str 
end 
...
def piged(word)
    ...
    (0...word.length-1).each do |i|
        return word[i..-1] + word[0...i] + "ay" if vowels.include?(word[i])
    end
end 
...
#     (sent.split.map.with_index {|word, i| i.even? ? remove_first(word) : remove_last(word) } ).join(' ')
# end
...
# # streaks of consecutive letters are translated to a single appearance of the letter followed 
# # by the number of times it appears in the streak. 
