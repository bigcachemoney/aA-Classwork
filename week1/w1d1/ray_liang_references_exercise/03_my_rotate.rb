...
def my_rotate!(array, amt)
    if amt > 0
        #first element to move to the last index
        amt.times do 
            remove_first_ele = array.shift #remove first ele
            array << remove_first_ele #append to end of array
        end
    else
        (-amt).times do
            #last element to move to the first index
            last_ele = array.pop #remove from the end
            array.unshift(last_ele) #unshift moves ele to the front of array
        end
    end
    array
end
